		...
		try {
			...
			try {
				const discrepanciesResponse = await fetch(
					"http://localhost:8080/api/frontend/events/with-discrepancies",
					{
						method: "GET",
						headers: {
							"Content-Type": "application/json"
			...
			setLoading(false)
			console.log(
				"Data fetching completed. Events:",
				events.length,
				"Discrepancies:",
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
				...
				<div className="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
					...
					<p className="text-xs text-yellow-700">
						Loading: {loading.toString()} | Events: {events.length} | Discrepancies:{" "}
						{discrepancies.length}
					</p>
				</div>
													...
													{event.articles && event.articles.length > 0 ? (
															...
															{event.articles.map(article => (
																...
																>
																	<div className="flex items-start justify-between mb-2">
																		<span
																			className={`badge ${getSourceColor(article.source)}`}
																		>
																			{article.source}
																		</span>
																		<a
														...
														<div className="bg-gray-50 rounded-lg p-4 border border-gray-200 text-center">
															<p className="text-sm text-gray-600">
																{event.articleCount} articles from
																various sources
															</p>
							...
							<div className="space-y-6">
								{discrepancies.length === 0 ? (
									...
									discrepancies.map((discrepancy, index) => (
										<div
											key={index}
											className="card border-l-4 border-l-orange-400"
										>
														...
														<div className="flex flex-wrap gap-2">
															...
															discrepancy.articles.length > 0 ? (
																...
																<span className="text-sm text-gray-500">
																	{discrepancy.articleCount}{" "}
																	sources with discrepancies
																</span>
															)}
														</div>
													</div>
