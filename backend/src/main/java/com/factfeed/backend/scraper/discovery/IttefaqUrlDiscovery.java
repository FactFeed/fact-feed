...
/**
 * URL discovery handler for Ittefaq - supports load more button functionality
 */
@Slf4j
@Component
public class IttefaqUrlDiscovery implements UrlDiscoveryHandler {

    private static final String SITE_NAME = "ittefaq";
    private static final String LATEST_PATH = "/online/latest-news";
    private static final String ARTICLE_SELECTOR = ".title a";
    ...
    public List<String> discoverUrls(WebDriver driver, String baseUrl, int maxCount, Set<String> stopUrls) {
        log.info("Starting URL discovery for Ittefaq, max count: {}", maxCount);
        ...
        try {
            ...
            while (discoveredUrls.size() < maxCount) {
                log.debug("Processing content batch {} for Ittefaq", loadAttempts + 1);
            ...
            log.info("URL discovery completed for Ittefaq. Discovered {} URLs in {} load attempts",
                    discoveredUrls.size(), loadAttempts + 1);
            return discoveredUrls;

        } catch (Exception e) {
            log.error("Error during URL discovery for Ittefaq", e);
